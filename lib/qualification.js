 ...
 *
 * ## Thresholds (Tiered Decision System)
 *
 * - **GO** (≥70 points): High-confidence match. Auto-advance to proposal queue.
 * - **EVALUATE** (50-69 points): Promising but needs one decision: "Do we have capacity?"
 * - **PASS** (<50 points): Not worth time. Logged for pattern analysis, not displayed by default.
 * - **NO-GO**: Automatic disqualification (missing required certifications/vehicles)
 *
 ...
 * @property {'GO' | 'NO-GO' | 'EVALUATE' | 'PASS'} status - Qualification status
...
export function qualifyOpportunity(opportunity, profile = null) {
    ...
    // =========================================================================
    // STEP 3: Determine final recommendation (Tiered System)
    // =========================================================================

    if (totalScore >= scoring.goThreshold) {
        // High-confidence match - auto-advance to proposal queue
        const topKeywords = matchedKeywords.slice(0, 2).join('/') || 'NAICS match';
        return {
            status: 'GO',
            reason: `High-confidence: ${topKeywords}`,
            recommendation: 'Pursue - add to proposal queue',
            score: totalScore,
            breakdown
        };
    }

    if (totalScore >= scoring.evaluateThreshold) {
        // Promising match - needs single decision: capacity
        const topKeywords = matchedKeywords.slice(0, 2).join('/') || 'partial match';
        return {
            status: 'EVALUATE',
            reason: `Promising: ${topKeywords} - check capacity`,
            recommendation: 'Evaluate - do we have bandwidth?',
            score: totalScore,
            breakdown
        };
    }

    // Low match - not worth pursuing
    return {
        status: 'PASS',
        reason: 'Low match - does not align with core capabilities',
        recommendation: 'Pass - below threshold',
        score: totalScore,
        breakdown
    };
}
 ...
 * @param {'GO' | 'NO-GO' | 'EVALUATE' | 'PASS' | 'all' | 'actionable'} status - Status to filter by
 * @returns {Array}
 */
export function filterByStatus(opportunities, status = 'all') {
    if (status === 'all') return opportunities;
    // 'actionable' returns GO + EVALUATE (things worth looking at)
    if (status === 'actionable') {
        return opportunities.filter(opp =>
            opp.qualification?.status === 'GO' ||
            opp.qualification?.status === 'EVALUATE'
        );
    }
